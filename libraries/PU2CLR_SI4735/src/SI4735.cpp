...
void SI4735::setFrequency(uint16_t freq)
{
    ...
    delay(maxDelaySetFrequency); // For some reason I need to delay here.  //临时测试 
}
...
void SI4735::frequencyUp()
{
    if (currentWorkFrequency >= currentMaximumFrequency)
        ...
        currentWorkFrequency= round(currentWorkFrequency/currentStep)*currentStep;   //增加频率自动对齐步进
...
void SI4735::seekStation(uint8_t SEEKUP, uint8_t WRAP)
{
    ...
    currentWorkFrequency= round((float)currentWorkFrequency/currentStep)*currentStep;   //增加频率自动对齐步进
...
void SI4735::seekStationProgress(void (*showFunc)(uint16_t f), uint8_t up_down)
{
    ...
    if (lastMode == SSB_CURRENT_MODE)
        return;
            ...
            seekStation(up_down, 0); //zooc 移出循环外
    do
    {
